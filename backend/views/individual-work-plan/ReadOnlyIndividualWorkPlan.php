...
<div class="corporate-work-plan-view">
    ...
    <?php if($model->ImmediateSupervisor == Yii::$app->user->identity->no): ?>
        <div class="headert" style="float: right;">
            <?= Html::a('Approve Work Plan', [
                '/individual-work-plan/approve',
                'IndividualWorkPlanId'=> $model->IndividualWorkPlanId], 
                ['class'=>'btn btn-success grid-button',
                'data-method' => 'POST'])
            ?>
            <?= Html::button('Reject Work Plan',
                    [  'value' => Url::to(['individual-work-plan/reject',
                    'IndividualWorkPlanId'=>$model->IndividualWorkPlanId,]),
                    'title' => 'Reject Workplan Reasons',
                    'class' => 'btn btn-danger  btn-flat border-warning showModalButton',
                    'data-method' => 'POST'
                    ]); 
            ?>
        </div>
